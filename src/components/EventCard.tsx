  ...
  return (
    <Link to={`/events/${event.id}`} className="card-hover overflow-hidden flex flex-col group">
      ...
      <div className="relative h-40 bg-blue-50 overflow-hidden">
        {event.image_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <svg className="w-14 h-14 text-blue-200" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2L2 9.5V22h7v-7h6v7h7V9.5L12 2z"/>
            </svg>
          </div>
        )}
        {/* Date badge */}
        <div className="absolute top-3 left-3 bg-white rounded-lg shadow px-2.5 py-1.5 text-center min-w-[44px]">
          <p className="text-[10px] font-bold leading-none" style={{ color: '#1e88e5' }}>{month}</p>
          <p className="text-lg font-extrabold text-gray-900 leading-none">{day}</p>
        </div>
        {event.is_free ? (
          <span className="badge absolute top-3 right-3 bg-green-100 text-green-700 shadow-sm">Free</span>
        ) : event.cost ? (
          <span className="badge absolute top-3 right-3 bg-white text-gray-700 shadow-sm">{event.cost}</span>
        ) : null}
      </div>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <h3 className="font-semibold text-gray-900 text-sm leading-snug mb-1 line-clamp-2 group-hover:text-blue-600 transition-colors" style={{ fontFamily: 'Poppins, sans-serif' }}>
          {event.title}
        </h3>
        <p className="text-xs font-medium mb-3" style={{ color: '#1e88e5' }}>{event.church_name}</p>
        <div className="mt-auto space-y-1">
          ...
          {(event.city || event.church_city) && (
            <div className="flex items-center gap-1.5 text-xs text-gray-500">
              ...
              <span>{event.city || event.church_city}{(event.state || event.church_state) ? `, ${event.state || event.church_state}` : ''}</span>
            </div>
          )}
        </div>
      </div>
